...
const PropertyTable = ({filterProperty  , handleEdit , handleDeleteModal}) => {
  ...
  return (
    <div className="m-6 overflow-hidden rounded-2xl shadow-sm  ">
      {/* TITLE */}
      <div className=" px-2 lg:px-4 py-2 font-semibold text-xs lg:text-sm text-gray-700  bg-yellow-500  ">
        Properties already in the name of this member
      </div>
      <div className="overflow-x-auto">
        <div className="min-w-[1000px]">
      {/* HEADER */}
      <div className="grid grid-cols-9  lg:text-base text-xs bg-blue-900 text-white font-semibold p-2  lg:p-3   ">
        <div>Sr.</div>
      ...
      <div className="bg-white">
        {filterProperty?.map((member, index) => (
          <div
            key={member.id}
            className="grid grid-cols-9 p-3  lg:text-base text-xs border-b hover:bg-[#f5f5f5] transition-colors duration-200"
          >
            <div>{index + 1}</div>
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className=" px-1 lg:px-2  py-0 lg:py-1 bg-green-500 text-white  text-xs lg:text-base rounded hover:scale-115 transition-all duration-200"
                title="Edit Property"
              >
                <TbHomeEdit />
              </button>
        ...
        {(!filterProperty || filterProperty.length === 0) && (
          <div className="text-center text-xs lg:text-base py-8 text-gray-500">
            No properties found
          </div>
        )}
      </div>
